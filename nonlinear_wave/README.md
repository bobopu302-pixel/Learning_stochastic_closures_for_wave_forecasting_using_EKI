...
## Changes vs the source tree (`3. KDV_nonlinear_case`)

Every shipped file carries a provenance docstring (`Origin:` +
`Changes vs origin:`).  Summary of what this release changed or
deliberately did **not** ship:

1. **Engine rewire.**  The hand-rolled EKI loops in `sw_eki_s1.main` and
   `sw_eki_h.main` were replaced by `algorithms.eki.run_eki` with:
   `ensemble_evaluator` (pool + CRN + N_G averaging),
   `failed_mask_fn`/`sentinel_row_fn` (spec failure rule),
   `stop_rel_tol=0.01`/`stop_patience=3` (spec stop),
   `post_update` (spec box clip), `iteration_callback` (S2 checkpoint),
   and `jitter=0.0` (reproducing the origin's un-regularised Kalman-gain
   solve; for Φ it is redundant, since the engine always computes Φ with
   the exact Γ by Cholesky whitening, independent of the jitter setting).
   Result files (`iter_NNN.npz`, `checkpoint.npz`, `summary.json` fields,
   gamma caches) keep their origin layout; summaries gained additive keys
   (`n_updates`, `stop_reason`, `phi_ensemble_final_evaluation`).  The
   saved Φ values are recomputed inside the drivers with the origin's
   explicit-inverse formula, so archived-file numbers are bit-identical
   to the origin's; the engine's internal Φ (Cholesky whitening of the
   exact Γ — it drives the stop rule) differs from the origin loop at
   the last-ulp level only.
2. **Spec-only configuration.**  The `SW_ALGO=legacy`, `SW_STOP_RULE`,
   `SW_GAMMA_MODE=legacy` env branches were deleted; the drivers hard-set
   `SW_GAMMA_TERMS=var_ref_only` and always use log-φ coordinates, CRN,
   N_G averaging, and final-ensemble-mean reporting.  Deleted with them:
   `joint_stop`/`spec_stop` helpers, the legacy Γ builders
   (`build_gamma_incr` and the `build_gamma_dense`/`base.build_gamma`
   call sites), the legacy per-member seed path, and the legacy
   stagnation/discrepancy stop knobs (`SDE_H_DISC_STOP`,
   `SDE_H_STAG_WINDOW`, `SDE_H_STAG_MIN_ITER`).
3. **S1b spline variant deleted** (`sw_truth.py`, `sw_eki_s1.py`): the
   spec chain observes only the S1a terrain-law truth; the spline truth
   read knots from a legacy results directory not shipped here.  Also
   deleted from `sw_eki_s1.py`: the `--classic` (q=44) and `--incr`
   (q=151) S1 observation options and `forward_statistics_incr` (the S2
   driver keeps `compute_statistics_incr` for its q=151 vector), and the
   frozen coarse-grid forward branch (spec is fine-grid only).
4. **Remote split deleted** (`sw_eki_h.py`): the `SW_REMOTE_HOST` lazy
   branch is gone; `v3_dist.py` and `v3_remote_worker.py` are not
   shipped.  The bit-identical path-parallel pool mode
   (`SW_H_PATH_PARALLEL=1`) is kept.
5. **Legacy reference-record mode deleted** (`sw_ref_records.py`): only
   the spec mode (own sea state + own baseline per record,
   `SW_REF_NEW_SEA_STATE=1`) is shipped; launching without it fails
   loudly.
6. **No plotting anywhere.**  Figure blocks were deleted from
   `sw_eki_s1.py`, `sw_eki_h.py`, `sw_gpr_reference.py`,
   `v3_s1_validation.py`, `v3_s2_validation.py`, `v3_s2_threeway.py`.
   Where a figure was the only holder of derived data, that data is now
   saved instead (`validation_profiles.npz`, `s2_surface_slices.npz`,
   `s2_convergence.npz`, `threeway_pdf_spectra.npz`, additive keys in
   `threeway_data.npz`).  Not shipped at all: `v3_s1_figures.py`,
   `v3_s2_figures.py`.
7. **Monitors and legacy chains not shipped:** `sw_status.py`,
   `v3_status.py`, `sw_fine_chain.sh`, `v3_chain.sh`,
   `v3_sync_local.sh`, `v3_sync_loop.sh`, and the probe/report scripts
   of retired investigation lines (`v3_devrms_probe.py`,
   `v3_stability_probe.py`, `v3_ng_report.py`, `v3_dist_check.py`).
8. `v3_spec_chain.sh`: paths adapted to this folder; the server-specific
   python autodetect replaced by a `PYTHON` override.
