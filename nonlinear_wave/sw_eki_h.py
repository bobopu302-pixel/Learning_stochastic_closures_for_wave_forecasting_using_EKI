...
Origin: 3. KDV_nonlinear_case/sw_eki_h.py
Changes vs origin:
- ENGINE REWIRE: the hand-rolled EKI loop in main() is replaced by
  algorithms.eki.run_eki.  Ensemble evaluation (pool + CRN spec_seed_root +
  N_G averaging) lives in an ensemble_evaluator; failed members follow the
  spec rule (all N_G realisations failed -> NaN row) via failed_mask_fn and
  are replaced by sentinel_row_fn = observation + 10*sqrt(diag(Gamma));
  spec stopping via stop_rel_tol=0.01 / stop_patience=3; the box clip is a
  post_update hook; jitter=0.0 keeps the origin's un-regularised Kalman-gain
  solve (for Phi it is redundant: the engine always computes Phi with the
  exact Gamma, by Cholesky whitening -- last-ulp different from the origin
  loop's explicit-inverse arithmetic, so the audit files recompute Phi with
  the origin inv-based formula and stay bit-identical to the origin's).
- CHECKPOINT/RESUME kept: the engine cannot restart mid-run by itself, so
  the driver manages resume state around ONE run_eki call -- the checkpoint
  restores the ensemble, the rng bit-generator state, the bookkeeping
  counters and the next iteration index, and the evaluator offsets the
  engine's iteration by start_iteration so CRN seeds and iter_NNN.npz file
  names continue exactly where the interrupted run stopped.  Checkpoints
  are written twice per iteration exactly like the origin: once from the
  evaluator (pre-update ensemble, so a stop leaves the evaluated state on
  disk) and once from iteration_callback (post-update ensemble).
  checkpoint.npz keys are unchanged.
- SPEC-ONLY: SW_ALGO / SW_STOP_RULE / SW_GAMMA_MODE env branches deleted
  (SPEC=True, unified var_ref-only Gamma, log coordinates for phi and the
  four GP hyper-parameters, spec stop, final-ensemble-mean reporting).
  Deleted with them: joint_stop / spec_stop (engine implements the rule),
  the legacy stopping env knobs (SDE_H_DISC_STOP / SDE_H_STAG_WINDOW /
  SDE_H_STAG_MIN_ITER), the legacy per-member seed path, the legacy
  build_gamma_incr Gamma and its gamma-cache resume load (the cache FILE is
  still written under its original name).
- deleted the SW_REMOTE_HOST / v3_dist remote-evaluation split in
  forward_statistics_h_batch (v3_dist.py / v3_remote_worker.py are not
  shipped); the path-parallel pool mode (SW_H_PATH_PARALLEL=1) is kept and
  remains bit-identical to the member-serial evaluation.
- deleted the matplotlib surface-slice figure at the end of main(); the
  ensemble needed to redraw it is in iter_NNN.npz, and
  v3_s2_validation.py saves the slice data to npz.
- s1_phiq_prior's default summary path now points at the spec S1 output
  (S1a_eki_dense_fine_diag); the chain always passes SW_H_S1_SUMMARY.
- as in sw_eki_s1: when the iteration cap is reached the engine evaluates
  the final ensemble once more (one extra iter_NNN.npz; reported separately
  in the summary).
